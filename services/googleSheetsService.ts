...
export async function fetchEditorialPlanning(): Promise<EditorialEvent[]> {
  const data = await fetchFromScript(PLANNING_SCRIPT_URL, { type: 'planning' });
  return data.map((item: any) => ({
    titre: item['Titre'] || '',
    type: item['Type'] || '',
    reseaux: item['Réseaux'] || '',
    datePublication: item['Date de publication'] || '',
    boost: item['Boost'] || 'FALSE',
    montantEngage: item['Montant engagé'] || '',
  })).filter((e: EditorialEvent) => e.datePublication !== '');
}
