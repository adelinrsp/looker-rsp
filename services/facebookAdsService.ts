...
export interface SocialPost {
  ...
  isVideo?: boolean;
  // Metrics d'engagement détaillés
  impressions?: number;
  reach?: number;
  reactions?: number;
  clicks?: number;
...
const PAGE_ID = '1690820184507800';
...
export async function fetchSocialFeed(): Promise<SocialPost[]> {
  try {
    ...
    if (!fbResult.error && fbResult.data) {
      fbPosts = await Promise.all(fbResult.data.map(async (post: any) => {
        let insights = {};
        try {
          const insightsUrl = `https://graph.facebook.com/v18.0/${post.id}/insights?metric=post_impressions,post_impressions_organic,post_engaged_users,post_engaged_fan,post_clicks,post_clicks_by_type,post_reactions_by_type_total,post_activity&access_token=${ACCESS_TOKEN}`;
          const insightsResponse = await fetch(insightsUrl);
          const insightsData = await insightsResponse.json();
          if (insightsData.error) {
            console.warn(`FB insights error for post ${post.id}:`, insightsData.error);
          } else if (insightsData.data) {
            insightsData.data.forEach((metric: any) => {
              insights = { ...insights, [metric.name]: metric.values?.[0]?.value || 0 };
            });
          }
        } catch (e) {
          console.error("Error fetching FB insights:", e);
        }

        const totalEngagement = (post.likes?.summary?.total_count || 0) + (post.comments?.summary?.total_count || 0) + (post.shares?.count || 0);
        ...
        return {
          ...
          reach: insights['post_impressions_organic'] || 0,
          reactions: insights['post_reactions_by_type_total'] || post.likes?.summary?.total_count || 0,
    ...
    console.log("Instagram business account response:", pageInfo);

    let igPosts: SocialPost[] = [];
    if (pageInfo.error) {
      console.error("Error fetching Instagram account:", pageInfo.error);
    } else if (pageInfo.instagram_business_account) {
      const igId = pageInfo.instagram_business_account.id;
      // Ajout de thumbnail_url et media_type pour gérer les vidéos
      const igUrl = `https://graph.facebook.com/v18.0/${igId}/media?fields=caption,media_url,thumbnail_url,media_type,permalink,timestamp,like_count,comments_count,ig_id&access_token=${ACCESS_TOKEN}&limit=50`;
      const igResponse = await fetch(igUrl);
      const igResult = await igResponse.json();

      if (!igResult.error && igResult.data) {
        igPosts = await Promise.all(igResult.data.map(async (post: any) => {
          let insights = {};
          try {
            const insightsUrl = `https://graph.facebook.com/v18.0/${post.id}/insights?metric=impressions,reach,profile_visits,clicks,engagement&access_token=${ACCESS_TOKEN}`;
            const insightsResponse = await fetch(insightsUrl);
            const insightsData = await insightsResponse.json();
            if (insightsData.error) {
              console.warn(`IG insights error for post ${post.id}:`, insightsData.error);
            } else if (insightsData.data) {
              insightsData.data.forEach((metric: any) => {
                insights = { ...insights, [metric.name]: metric.values?.[0]?.value || 0 };
              });
